...
public class DashboardController(ApplicationDbContext _ctx) : ControllerBase
{
    ...
    public async Task<ActionResult<List<RecentActivityDto>>> GetRecentActivity()
    {
        var activities = await ctx.Activity
            ...
            .Select(a => new RecentActivityDto
            {
                ...
                Type = a.Type,
                Subject = a.Subject,
                EntityName = a.Customer != null ? a.Customer.Name
                           : a.Contact != null ? a.Contact.Name
                           : a.Lead != null ? a.Lead.Company
                           : a.Opportunity != null ? a.Opportunity.Name
                           : null,
                Date = a.ActivityDate
            })
            .ToListAsync();
    ...
    public async Task<ActionResult<List<TimelineEventDto>>> GetTimeline(string entityType, long entityId)
    {
        ...
        // Get activities linked to this entity
        var activitiesQuery = entityType.ToLower() switch
        {
            "customer" => ctx.Activity.Where(a => a.CustomerId == entityId),
            "contact" => ctx.Activity.Where(a => a.ContactId == entityId),
            "lead" => ctx.Activity.Where(a => a.LeadId == entityId),
            "opportunity" => ctx.Activity.Where(a => a.OpportunityId == entityId),
            _ => ctx.Activity.Where(a => false)
        };
        ...
        // Get sales linked to this entity (customers only)
        if (entityType.Equals("customer", StringComparison.OrdinalIgnoreCase))
        {
            var sales = await ctx.Sale
                .Where(s => s.CustomerId == entityId)
            ...
            events.AddRange(sales.Select(s => new TimelineEventDto
            {
                Id = s.Id,
                EventType = "Sale",
                Title = $"Sale - {s.TotalAmount:C}",
                Description = s.Notes,
                Date = s.SaleDate
            }));
        }

        // Get support cases linked to this entity
        if (entityType.Equals("customer", StringComparison.OrdinalIgnoreCase) ||
            entityType.Equals("contact", StringComparison.OrdinalIgnoreCase))
        {
            var casesQuery = entityType.Equals("customer", StringComparison.OrdinalIgnoreCase)
                ? ctx.SupportCase.Where(s => s.CustomerId == entityId)
                : ctx.SupportCase.Where(s => s.ContactId == entityId);
        ...
        // Get communications linked to this entity
        var commsQuery = entityType.ToLower() switch
        {
            "customer" => ctx.Communication.Where(c => c.CustomerId == entityId),
            "contact" => ctx.Communication.Where(c => c.ContactId == entityId),
            "lead" => ctx.Communication.Where(c => c.LeadId == entityId),
            "opportunity" => ctx.Communication.Where(c => c.OpportunityId == entityId),
            _ => ctx.Communication.Where(c => false)
        };

        var comms = await commsQuery
            .OrderByDescending(c => c.CommunicationDate)
            .Take(20)
            .ToListAsync();

        events.AddRange(comms.Select(c => new TimelineEventDto
        {
            Id = c.Id,
            EventType = c.Type,
            Title = c.Subject,
        ...
        return Ok(events.OrderByDescending(e => e.Date).ToList());
    }
}
